...
@section('content')
<div id="editBrandModal" class="bg-white items-center justify-center">
    <div class="bg-white border border-gray-300 rounded-xl shadow-2xl w-full p-6 max-w-lg mx-auto">
        <h2 class="text-2xl font-semibold text-gray-800 mb-6 border-b pb-2 text-center">Edit Brand</h2>
        ...
        <form action="{{ route('admin.brands.update', $brand->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
           ...
           <div class="mb-4">
                ...
                @enderror
                <div class="mt-3">
                    <img id="image-preview" src="{{ $brand->image ? Storage::url($brand->image) : 'https://media.istockphoto.com/id/1324356458/vector/picture-icon-photo-frame-symbol-landscape-sign-photograph-gallery-logo-web-interface-and.jpg?s=612x612&w=0&k=20&c=ZmXO4mSgNDPzDRX-F8OKCfmMqqHpqMV6jiNi00Ye7rE=' }}"
                         class="w-24 h-24 rounded-md border border-gray-300 object-cover {{ $brand->image ? '' : '' }}" alt="Preview">
                </div>
            </div>
...
<script>
    ...
    document.getElementById('image').addEventListener('change', function (event) {
        ...
        const defaultSrc = '{{ $brand->image ? Storage::url($brand->image) : asset('images/placeholder.jpg') }}';
